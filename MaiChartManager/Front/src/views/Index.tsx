...
export default defineComponent({
  setup() {
    ...
    const modManagerRef = ref<HTMLElement>();
    const genreManagerRef = ref<HTMLElement>();
    const batchActionRef = ref<HTMLElement>();

    watch(sidebarActive, (val) => {
      if (val === 'charts') return;
      const refMap: Record<string, typeof modManagerRef> = {
        mods: modManagerRef,
        genres: genreManagerRef,
        batch: batchActionRef,
      };
      const target = refMap[val];
      sidebarActive.value = 'charts';
      nextTick(() => {
        target?.value?.querySelector('button')?.click();
      });
    });
    ...
    return () => (
      <div class="flex justify-center" ref={mainDivRef}>
        <DragDropDispatcher />
        {/* Hidden containers for sidebar-triggered components */}
        <div class="hidden" ref={modManagerRef}><ModManager /></div>
        <div class="hidden" ref={genreManagerRef}><GenreVersionManager /></div>
        <div class="hidden" ref={batchActionRef}><BatchActionButton /></div>
        <div class="grid cols-[48px_40em_1fr] w-[min(100rem,100%)] max-[1440px]:cols-[48px_1fr] max-[767px]:cols-1">
